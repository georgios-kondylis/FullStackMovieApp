...
type Props =  {
  setCreatingProfileIsActive: React.Dispatch<React.SetStateAction<boolean>>
  text: string
}

const CreateProfileCard = ({ setCreatingProfileIsActive, text}: Props) => {
  ...
  return (
    <div className={`flex flex-col gap-3 items-center`} onClick={() => navigate('/profiles/createProfile')}>
      ...
      text-[8rem] text-[gray] hover:text-[#eaeaea] cursor-pointer`} 
            onClick={() => setCreatingProfileIsActive(true)}
      >
